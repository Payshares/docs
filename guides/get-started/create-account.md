...
The first thing you’ll need to do anything on the Payshares network is an account. Accounts hold all your money inside Payshares and allow you to send and receive payments—in fact, pretty much everything in Payshares is in some way tied to an account.

Every Payshares account has a **public key** and a **secret seed**. Payshares uses public key cryptography to ensure that every transaction is secure. The public key is always safe to share—other people need it to identify your account and verify that you authorized a transaction. The seed, however, is private information that proves you own your account. You should never share the seed with anyone. It’s kind of like the combination to a lock—anyone who knows the combination can open the lock. In the same way, anyone who knows your account’s seed can control your account.

If you’re familiar with public key cryptography, you might be wondering how the seed differs from a private key. The seed is actually the single secret piece of data that is used to generate both the public and private key for your account. Payshares’s tools use the seed instead of the private key for convenience: To have full access to an account, you only need to provide a seed instead of both a public key and a private key.[^1]

Because the seed must be kept secret, the first step in creating an account is creating your own seed and key—when you finally create the account, you’ll send only the public key to a Payshares server. You can generate the seed and key with the following command:

<code-example name="Generating Keys">

```js
// create a completely new and unique pair of keys
// see more about KeyPair objects: https://payshares.github.io/js-payshares-sdk/Keypair.html
var pair = PaysharesSdk.Keypair.random();
...
```java
// create a completely new and unique pair of keys.
// see more about KeyPair objects: https://payshares.github.io/java-payshares-sdk/org/payshares/sdk/KeyPair.html
import org.payshares.sdk.KeyPair;
...
Now that you have a seed and public key, you can create an account. In order to prevent people from making a huge number of unnecessary accounts, each account must have a minimum balance of 1 stakk (stakks are the built-in currency of the Payshares network).[^2] Since you don’t yet have any stakks, though, you can’t pay for an account. In the real world, you’ll usually pay an exchange that sells stakks in order to create a new account.[^3] On Payshares’s test network, however, you can ask Friendbot, our friendly robot with a very fat wallet, to create an account for you.
...
request.get({
  url: 'https://horizon-testnet.payshares.org/friendbot',
...
String friendbotUrl = String.format(
  "https://horizon-testnet.payshares.org/friendbot?addr=%s",
  pair.getAccountId());
...
func main() {
	...
	resp, err := http.Get("https://horizon-testnet.payshares.org/friendbot?addr=" + address)
	if err != nil {
		log.Fatal(err)
	}
...
```java
import org.payshares.sdk.Server;
import org.payshares.sdk.responses.AccountResponse;
...
<div class="sequence-navigation">
  <a class="button button--previous" href="index.html">Back: Payshares Network Overview</a>
...
[^2]: Other features of Payshares, like [trust lines](../concepts/assets.md#trustlines), require higher minimum balances. For more on minimum balances, see [fees](../concepts/fees.md#minimum-account-balance)

[^3]: CoinMarketCap maintains a list of exchanges that sell stakks at http://coinmarketcap.com/currencies/payshares/#markets
