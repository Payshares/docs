...
**XDR**, also known as _External Data Representation_, is used throughout the Payshares network and protocol.  The ledger, transactions, results, history, and even the messages passed between computers running payshares-core are encoded using XDR.
...
Since XDR is a binary format and not as widely known as simpler formats like JSON, the Payshares SDKs all include tools for parsing XDR and will do so automatically when retrieving data.
